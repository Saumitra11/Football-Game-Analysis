...
st.sidebar.markdown("""Select one of the given Football Matches for a detailed analysis: """)
st.sidebar.markdown(""" 
...
filename = 'open-data-master/data/events/'+str(games_dict.get(menu_game))+'.json'
...
activities = ['Pass', 'Ball Received', 'Carry', 'Pressure', 'Shot', 'HeatMap']
...
st.sidebar.markdown('For obtaining statistic plot, select a player:')


# Titles and text above the pitch
st.title('Euros 2020 Knockouts Games Analysis')
st.markdown("""
The knockout stage of UEFA Euro 2020 was held between June 26 and July 11, 2021. 
It consisted of 15 matches between 16 teams that advanced from the group round. 
Italy defeated England on penalties in the final game in London, claiming the title for the second time in their history.
""")
...
# Define six functions for six activities from drop-down menu
# Pass plot function
def pass_map():
...
# HeatMap
def heat_map():
    kalvin_action = df[(df['player_name']== menu_player)][["id", "type_name","period", "timestamp", "location"]] 
    ...
    color = 'blue' if menu_team == team_1 else 'red'
    if(menu_team == team_1):
        sns.kdeplot(x=x_coord, y=y_coord, shade = "True", color = color, n_levels = 30)
    else:
        sns.kdeplot(x=x_coord, y=y_coord, shade = "True", color = color, n_levels = 30)
...
if menu_activity == 'Pass':
    ax = pass_map()
elif menu_activity == 'Ball Received':
    ax = ball_receipt_map()
...
st.markdown("""
## For Data Visualization Project
by
